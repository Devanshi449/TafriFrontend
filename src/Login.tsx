...
const Login = () => {
    ...
    const signInWithEmail = async () => {
        ...
        // Send the login request to the ASP.NET API
        const loginData = {
            email: email,
            password: password,
        };
        ...
        try {
            const response = await axios.post('/proxy/User/UserLogin', loginData);
            const userData = response.data;
            console.log(userData);
    
            // Store user details in local storage or session storage
            sessionStorage.setItem('userId', userData.userId);
            sessionStorage.setItem('userName', userData.name);
            sessionStorage.setItem('userRole', userData.role);
    
            // Redirect to respective page based on user role
            ...
            if (axiosError.response && axiosError.response.data) {
                const errorMessage = typeof axiosError.response.data === 'string'
                    ...
                    : JSON.stringify(axiosError.response.data); // Convert to string if not
